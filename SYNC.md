...
| `src/app/domain/services` + `src/app/tools/modeler/services` | `src/modeler/service/*` plus `src/modeler/infrastructure/{DiagramJsModelerAdapter,DiagramJsIconAdapter,EditorSessionOwner}.ts` and `src/modeler/infrastructure/color-picker/*` (ModelerService/InitializerService ≈ `EgonClient` and the active-session adapters/owner; candidate-session staging, the client-scoped picker coordinator/passive preview state, provider controller, and bundled Preact picker have no upstream counterpart)                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `src/app/tools/icon-set-config/{domain,services}`            | `src/iconSet/{domain,service,infrastructure}` (`src/iconSet/domain/defaultIcons.ts` is local, immutable artwork derived from the former demo `Person`/`Document` SVG files; it has no upstream catalog counterpart)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
- **The document-global color-picker bridge had no client or request ownership.**
  ...
  ADR 0035 now routes that coordinator through a per-client final-result
  provider, with a bundled default picker and `false` mode; the public request
  events and imperative response methods are gone. Temporary colors still live
  in passive renderer preview state and confirmation still uses the existing
  commands. A future WPS host extraction must implement the provider contract
  and retain originating client/request identity rather than recreate the
  global seam. Locked by `ColorPickerCoordinator.spec.ts`,
  `ColorPickerController.spec.ts`, the picker browser specs, and the migrated
  context-pad tests.
...
- **`selectedElement` was never cleared, so the host's color reply could recolor
  ...
  the picker._ Superseded locally by ADR 0035: the context-pad provider retains
...
- **Activity-label selection iterated one waypoint past the segment list.**
  ...
  out-of-range cases in `modeler/domain/labeling/__tests__/position.spec.ts`. Issue
